...
def safe_termination_str(termination) -> str:
    """Преобразует терминацию в безопасное имя файла."""
...
def save_csv(metadata, substrate, film, output_dir: str):
    """Сохраняет CSV summary."""
    import csv
    if not metadata:
        logger.warning("No metadata to save — CSV skipped.")
        return
    csv_path = Path(output_dir) / f"{substrate.composition.reduced_formula}_{film.composition.reduced_formula}_summary.csv"
...
def _process_match(substrate_bulk, film_bulk, match_record, config, output_dir: str, logger):
    ...
    for termination in cib.terminations:
        ...
        # Фильтрация по физическим критериям
        filtered_interfaces = []
        for iface in term_interfaces:
            sub_density = compute_surface_density(iface.substrate, select="top", layer_thickness=config.surface_thickness)
            film_density = compute_surface_density(iface.film, select="bottom", layer_thickness=config.surface_thickness)
            sub_charge = compute_surface_charge_density(iface.substrate, select="top", layer_thickness=config.surface_thickness)
            film_charge = compute_surface_charge_density(iface.film, select="bottom", layer_thickness=config.surface_thickness)
            total_charge = abs(sub_charge + film_charge)

            passed = (sub_density > config.density_limit and
                      film_density > config.density_limit and
                      total_charge < config.charge_limit)

            filtered_interfaces.append((iface, sub_density, film_density, sub_charge, film_charge, total_charge, passed))

        if not filtered_interfaces:
            continue

        # Берем интерфейс с минимальным num_sites
        iface, sub_density, film_density, sub_charge, film_charge, total_charge, passed = min(filtered_interfaces, key=lambda x: x[0].num_sites)
        term_str = safe_termination_str(termination)
        filename = f"{sub_formula}_{film_formula}_{''.join(map(str, sub_hkl))}_{''.join(map(str, film_hkl))}_{term_str}_{iface.num_sites}at"
        out_subdir = Path(output_dir) / f"{film_formula}_on_{sub_formula}" / f"{''.join(map(str, sub_hkl))}_{''.join(map(str, film_hkl))}"
...
def build_interfaces(substrate_bulk, film_bulk, match_record, config, output_dir="interfaces", logger=None):
    if logger is None:
        logger = logging.getLogger(__name__)

    # logger.info(f"Processing match: {match_record['hkl_sub']}/{match_record['hkl_film']}, von Mises={match_record['von_mises']*100:.2f}%")
    results_all, accepted_all = _process_match(substrate_bulk, film_bulk, match_record, config, output_dir, logger)

    # Сортировка: сначала по total charge, потом по num_sites (макс), потом по von Mises
    results_all.sort(key=lambda x: (x["abs_charge_density"], -x["num_sites"], x["von_mises"]))
